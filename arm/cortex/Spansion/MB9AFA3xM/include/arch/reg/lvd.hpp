...
struct LVD
{
   ...
   */
  struct LVD_CTL
  : public reg< uint16_t, base_addr + 0x0, rw, 0x8400 >
  {
    using type = reg< uint16_t, base_addr + 0x0, rw, 0x8400 >;

    using LVDRE  = regbits< type, 15,  1 >;  /**< Low-voltage detection reset operation enable bit           */
    using SVHR   = regbits< type, 10,  4 >;  /**< Low-voltage detection reset voltage setting bits           */
    using LVDIE  = regbits< type,  7,  1 >;  /**< Low-voltage detection interrupt enable bit                 */
    using SVHI   = regbits< type,  2,  4 >;  /**< Low-voltage detection interrupt voltage setting bits       */
    using LVDIM  = regbits< type,  1,  1 >;  /**< Low-voltage detection interrupt low power mode select bit  */
  };
