...
struct CRTRIM
{
   ...
   */
  struct MCR_FTRM
  : public reg< uint16_t, base_addr + 0x4, rw, 0x0200 >
  {
    using type = reg< uint16_t, base_addr + 0x4, rw, 0x0200 >;
   ...
   */
  struct MCR_TTRM
  : public reg< uint8_t, base_addr + 0x8, rw, 0x0F >
  {
    using type = reg< uint8_t, base_addr + 0x8, rw, 0x0F >;

    using TRT  = regbits< type,  0,  5 >;  /**< Temperature trimming setup bits  */
  };
