...
struct MFT0
{
  ...
  : public reg< uint16_t, base_addr + 0x14a, rw, 0x0000 >
  {
    ...
    using MSPC  = regbits< type, 12,  4 >;  /**< Reads the current counter value from a Peak value detection mask counter  */
    using MSZC  = regbits< type,  8,  4 >;  /**< Reads the current counter value from a Zero value detection mask counter  */
    using MSPI  = regbits< type,  4,  4 >;  /**< Sets the number of masked Peak value detections                           */
    using MSZI  = regbits< type,  0,  4 >;  /**< Sets the number of masked Zero value detections                           */
  };
   ...
   */
  struct FRT_TCAL
  : public reg< uint32_t, base_addr + 0x164, rw, 0x0000FFFF >
  {
    using type = reg< uint32_t, base_addr + 0x164, rw, 0x0000FFFF >;
   ...
   */
  struct OCU_OCSC
  : public reg< uint8_t, base_addr + 0x125, rw, 0x00 >
  {
    using type = reg< uint8_t, base_addr + 0x125, rw, 0x00 >;

    using MOD5  = regbits< type,  5,  1 >;  /**< OCSC.MOD4 and OCSC.MOD5 determine the operation mode of OCU ch.4/ch.5 in combination with OCSB54.CMOD  */
    using MOD4  = regbits< type,  4,  1 >;  /**< OCSC.MOD4 and OCSC.MOD5 determine the operation mode of OCU ch.4/ch.5 in combination with OCSB54.CMOD  */
    using MOD3  = regbits< type,  3,  1 >;  /**< OCSC.MOD2 and OCSC.MOD3 determine the operation mode of OCU ch.2/ch.3 in combination with OCSB32.CMOD  */
    using MOD2  = regbits< type,  2,  1 >;  /**< OCSC.MOD2 and OCSC.MOD3 determine the operation mode of OCU ch.2/ch.3 in combination with OCSB32.CMOD  */
    using MOD1  = regbits< type,  1,  1 >;  /**< OCSC.MOD0 and OCSC.MOD1 determine the operation mode of OCU ch.0/ch.1 in combination with OCSB10.CMOD  */
    using MOD0  = regbits< type,  0,  1 >;  /**< OCSC.MOD0 and OCSC.MOD1 determine the operation mode of OCU ch.0/ch.1 in combination with OCSB10.CMOD  */
  };
  ...
  : public reg< uint16_t, base_addr + 0x1a4, rw, 0x0000 >
  {
    ...
    using DMOD  = regbits< type, 12,  2 >;  /**< Specifies polarity for RTO(0) and RTO(1) signal outputs                 */
  ...
  : public reg< uint16_t, base_addr + 0x1b0, rw, 0x0000 >
  {
    ...
    using TMIE54  = regbits< type, 14,  1 >;  /**< Starts WFG54 reload timer and checks the operation state of it       */
    ...
    using TMIE32  = regbits< type, 10,  1 >;  /**< Starts WFG32 reload timer and checks the operation state of it       */
  ...
  : public reg< uint16_t, base_addr + 0x1b4, rw, 0x0000 >
  {
    ...
    using DTIEB  = regbits< type,  5,  1 >;  /**< Selects whether to set the WFIR.DTIFB flag for the path from the DTTIX pin for input signal to an analog noise filter  */
    ...
    using DTIEA  = regbits< type,  0,  1 >;  /**< Selects whether the WFIR.DTIFA register is set for the path via digital noise filter from the DTTIX input pin          */
  };
  ...
  : public reg< uint16_t, base_addr + 0x1ba, rw, 0x0000 >
  {
    ...
    using ACMP  = regbits< type,  0, 16 >;  /**< Specifies an AD conversion start time  */
  };
