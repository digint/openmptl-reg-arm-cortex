...
struct WC
{
  ...
  : public reg< uint8_t, base_addr + 0x0, ro, 0x00 >
  {
    ...
    using CTR  = regbits< type,  0,  6 >;  /**< Counter read bits  */
  };
  ...
  : public reg< uint8_t, base_addr + 0x1, rw, 0x00 >
  {
    ...
    using RLC  = regbits< type,  0,  6 >;  /**< Counter reload value setting bits  */
  };
   ...
   */
  struct WCCR
  : public reg< uint8_t, base_addr + 0x2, rw, 0x00 >
  {
    using type = reg< uint8_t, base_addr + 0x2, rw, 0x00 >;
  ...
  : public reg< uint16_t, base_addr + 0x10, rw, 0x0000 >
  {
    ...
    using SEL_OUT  = regbits< type,  8,  3 >;  /**< Output clock selection bit  */
    using SEL_IN   = regbits< type,  0,  2 >;  /**< Input clock selection bit   */
  };
