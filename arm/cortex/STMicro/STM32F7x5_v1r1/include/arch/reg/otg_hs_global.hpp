...
struct OTG_HS_GLOBAL
{
  ...
  : public reg< uint32_t, base_addr + 0x10, rw, 0x20000000 >
  {
    ...
    using DMAREQ   = regbits< type, 30,  1 >;  /**< DMA request signal enabled for USB OTG HS  */
  };
  ...
  : public reg< uint32_t, base_addr + 0x18, rw, 0x0 >
  {
    ...
    using RSTDE            = regbits< type, 23,  1 >;  /**< Reset detected interrupt mask                        */
    using LPMINTM          = regbits< type, 27,  1 >;  /**< LPM interrupt mask                                   */
  };
