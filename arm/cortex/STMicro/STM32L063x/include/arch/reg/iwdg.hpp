...
struct IWDG
{
  ...
  : public reg< uint32_t, base_addr + 0x0, wo, 0x00000000 >
  {
    ...
    using KEY  = regbits< type,  0, 16 >;  /**< Key value (write only, read 0x0000)  */
  };
  ...
  : public reg< uint32_t, base_addr + 0xc, ro, 0x00000000 >
  {
    ...
    using WVU  = regbits< type,  2,  1 >;  /**< Watchdog counter window value update  */
   ...
   */
  struct WINR
  : public reg< uint32_t, base_addr + 0x10, rw, 0x00000FFF >
  {
    using type = reg< uint32_t, base_addr + 0x10, rw, 0x00000FFF >;

    using WIN  = regbits< type,  0, 12 >;  /**< Watchdog counter window value  */
  };
};
} // namespace mptl
