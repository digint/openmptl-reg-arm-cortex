...
struct SIM
{
   ...
   */
  struct SOPT1
  : public reg< uint32_t, base_addr + 0, rw, 0x8000F03F >
  {
    using type = reg< uint32_t, base_addr + 0, rw, 0x8000F03F >;
  ...
  : public reg< uint32_t, base_addr + 0x1028, rw, 0 >
  {
    ...
    using OPAMP   = regbits< type, 21,  1 >;  /**< OPAMP Clock Gate Control   */
    using TRIAMP  = regbits< type, 24,  1 >;  /**< TRIAMP Clock Gate Control  */
  };
  ...
  : public reg< uint32_t, base_addr + 0x1030, rw, 0 >
  {
    ...
    using SLCD  = regbits< type, 30,  1 >;  /**< Segment LCD Clock Gate Control  */
  };
   ...
   */
  struct SCGC4
  : public reg< uint32_t, base_addr + 0x1034, rw, 0xE0100030 >
  {
    using type = reg< uint32_t, base_addr + 0x1034, rw, 0xE0100030 >;
  ...
  : public reg< uint32_t, base_addr + 0x1038, rw, 0x40182 >
  {
    ...
    using LPTIMER  = regbits< type,  0,  1 >;  /**< Low Power Timer Access Control  */
