...
struct DCDC
{
   ...
   */
  struct REG3
  : public reg< uint32_t, base_addr + 0xc, rw, 0xAA46 >
  {
    using type = reg< uint32_t, base_addr + 0xc, rw, 0xAA46 >;
    ...
    using DCDC_VDD1P5CTRL_TRG_BUCK        = regbits< type,  6,  5 >;  /**< Target value of VDD1P5 in buck mode, 25 mV each step from 0x00 to 0x0F                            */
    using DCDC_VDD1P5CTRL_TRG_BOOST       = regbits< type, 11,  5 >;  /**< Target value of VDD1P5 in boost mode, 25 mV each step from 0x00 to 0x0F                           */
    using DCDC_VDD1P5CTRL_ADJTN           = regbits< type, 17,  4 >;  /**< Adjust value of duty cycle when switching between VDD1P5 and VDD1P8. The unit is 1/32 or 3.125%.  */
    ...
    using DCDC_VDD1P5CTRL_DISABLE_STEP    = regbits< type, 29,  1 >;  /**< Disable stepping for VDD1P5. Must set this bit before enter low power modes.                      */
  ...
  : public reg< uint32_t, base_addr + 0x10, rw, 0 >
  {
    ...
    using DCDC_SW_SHUTDOWN  = regbits< type,  0,  1 >;  /**< Shut down DCDC in buck mode. DCDC can be turned on by pulling PSWITCH to high momentarily (min 50 ms).DCDC_SW_SHUTDOWN should not be used in boost mode because when user write this bit, MCU won't be POR and enters an abnormal state.  */
