...
struct DMA
{
  ...
  : public reg< uint32_t, base_addr + 0, rw, 0 >
  {
    ...
    using GRP0PRI  = regbits< type,  8,  2 >;  /**< Channel Group 0 Priority                */
    using GRP1PRI  = regbits< type, 10,  2 >;  /**< Channel Group 1 Priority                */
  ...
  : public reg< uint32_t, base_addr + 0x4, ro, 0 >
  {
    ...
    using ERRCHN  = regbits< type,  8,  5 >;  /**< Error Channel Number or Cancelled Channel Number  */
  ...
  : public reg< uint8_t, base_addr + 0x100, rw, 0 >
  {
    ...
    using DPA     = regbits< type,  6,  1 >;  /**< Disable preempt ability. This bit resets to zero.    */
    using ECP     = regbits< type,  7,  1 >;  /**< Enable Channel Preemption. This bit resets to zero.  */
  };
  ...
  : public reg< uint16_t, base_addr + 0x101c, rw, 0 >
  {
    ...
    using MAJORLINKCH  = regbits< type,  8,  5 >;  /**< Link Channel Number                                       */
