...
struct FLEXIO
{
   ...
   */
  struct VERID
  : public reg< uint32_t, base_addr + 0, ro, 0x1010000 >
  {
    using type = reg< uint32_t, base_addr + 0, ro, 0x1010000 >;
   ...
   */
  struct PARAM
  : public reg< uint32_t, base_addr + 0x4, ro, 0x4080404 >
  {
    using type = reg< uint32_t, base_addr + 0x4, ro, 0x4080404 >;
  ...
  : public reg< uint32_t, base_addr + 0xc, ro, 0 >
  {
    ...
    using PDI  = regbits< type,  0,  8 >;  /**< Pin Data Input  */
  };
