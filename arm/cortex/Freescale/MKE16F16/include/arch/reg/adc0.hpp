...
struct ADC0
{
   ...
   */
  struct BASE_OFS
  : public reg< uint32_t, base_addr + 0x98, rw, 0x43 >
  {
    using type = reg< uint32_t, base_addr + 0x98, rw, 0x43 >;
  ...
  : public reg< uint32_t, base_addr + 0x9c, rw, 0 >
  {
    ...
    using OFS_ = regbits< type,  0, 12 >;  /**< Offset Error Correction Value  */
  };

  /**
   * ADC USER Offset Correction Register
   */
  struct USR_OFS
   ...
   */
  struct XOFS
  : public reg< uint32_t, base_addr + 0xa4, rw, 0x6 >
  {
    using type = reg< uint32_t, base_addr + 0xa4, rw, 0x6 >;
  ...
  : public reg< uint32_t, base_addr + 0xbc, rw, 0xB8 >
  {
    ...
    using CLP2_ = regbits< type,  0,  9 >;  /**< Calibration Value  */
  };
   ...
   */
  struct CLPX_OFS
  : public reg< uint32_t, base_addr + 0xe4, rw, 0x443 >
  {
    using type = reg< uint32_t, base_addr + 0xe4, rw, 0x443 >;
   ...
   */
  struct CLP9_OFS
  : public reg< uint32_t, base_addr + 0xe8, rw, 0x243 >
  {
    using type = reg< uint32_t, base_addr + 0xe8, rw, 0x243 >;
