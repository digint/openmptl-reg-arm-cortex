...
struct I2C0
{
  ...
  : public reg< uint8_t, base_addr + 0x1, rw, 0 >
  {
    ...
    using MULT  = regbits< type,  6,  2 >;  /**< Multiplier Factor  */
  };
  ...
  : public reg< uint8_t, base_addr + 0x6, rw, 0 >
  {
    ...
    using FLT_    = regbits< type,  0,  4 >;  /**< I2C Programmable Filter Factor          */
    using STARTF  = regbits< type,  4,  1 >;  /**< I2C Bus Start Detect Flag               */
    using SSIE    = regbits< type,  5,  1 >;  /**< I2C Bus Stop or Start Interrupt Enable  */
  ...
  : public reg< uint8_t, base_addr + 0xa, rw, 0 >
  {
    ...
    using SSLT  = regbits< type,  0,  8 >;  /**< Most significant byte of SCL low timeout value that determines the timeout period of SCL low.  */
  };
  ...
  : public reg< uint8_t, base_addr + 0xb, rw, 0 >
  {
    ...
    using SSLT  = regbits< type,  0,  8 >;  /**< Least significant byte of SCL low timeout value that determines the timeout period of SCL low.  */
  };
   ...
   */
  struct S2
  : public reg< uint8_t, base_addr + 0xc, rw, 0x1 >
  {
    using type = reg< uint8_t, base_addr + 0xc, rw, 0x1 >;

    using EMPTY  = regbits< type,  0,  1 >;  /**< Empty flag  */
    using ERROR  = regbits< type,  1,  1 >;  /**< Error flag  */
  };
};
} // namespace mptl
