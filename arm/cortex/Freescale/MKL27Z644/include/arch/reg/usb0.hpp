...
struct USB0
{
  ...
  : public reg< uint8_t, base_addr + 0x88, rw, 0 >
  {
    ...
    using CRC5    = regbits< type,  1,  1 >;  /**< This error interrupt has two functions                                                                                                                     */
  ...
  : public reg< uint8_t, base_addr + 0x94, rw, 0 >
  {
    ...
    using TXSUSPENDTOKENBUSY  = regbits< type,  5,  1 >;  /**< In Target mode, TXD_SUSPEND is set when the SIE has disabled packet transmission and reception              */
  ...
  : public reg< uint8_t, base_addr + 0xc0, rw, 0 >
  {
    ...
    using EPTXEN    = regbits< type,  2,  1 >;  /**< This bit, when set, enables the endpoint for TX transfers. See #aal353jj                            */
    using EPRXEN    = regbits< type,  3,  1 >;  /**< This bit, when set, enables the endpoint for RX transfers. See #aal353jj                            */
  ...
  : public reg< uint8_t, base_addr + 0x104, ro, 0x50 >
  {
    ...
    using DMPD  = regbits< type,  4,  1 >;  /**< Provides observability of the D- Pulldown signal output from USB.  */
    using DPPD  = regbits< type,  6,  1 >;  /**< Provides observability of the D+ Pulldown signal output from USB.  */
    using DPPU  = regbits< type,  7,  1 >;  /**< Provides observability of the D+ Pullup signal output from USB .   */
  };
  ...
  : public reg< uint8_t, base_addr + 0x108, rw, 0 >
  {
    ...
    using DPPULLUPNONOTG  = regbits< type,  4,  1 >;  /**< Provides control of the DP Pullup in USB, if USB is configured in non-OTG device mode.  */
  };
  ...
  : public reg< uint8_t, base_addr + 0x124, rw, 0x8 >
  {
    ...
    using OWN_OVERRD_EN    = regbits< type,  1,  1 >;  /**< When set to 1, during KEEP_ALIVE mode, if received token is not SETUP, the OWN bit of current BD will be forced to 0, so usb core will response with NAK  */
    using STOP_ACK_DLY_EN  = regbits< type,  2,  1 >;  /**< When set to 1, will respond the ipg_stop when usb core state is idle and no AHB transfer; when set to 0, will respond the ipg_stop when no AHB transfer   */
    using AHB_DLY_EN       = regbits< type,  3,  1 >;  /**< Must be set to 1 to delay the first USB AHB bus transfer until after the USB has exited KEEP_ALIVE mode                                                   */
  ...
  : public reg< uint8_t, base_addr + 0x144, rw, 0x1 >
  {
    ...
    using IRC_EN  = regbits< type,  1,  1 >;  /**< IRC48M enable  */
  };
