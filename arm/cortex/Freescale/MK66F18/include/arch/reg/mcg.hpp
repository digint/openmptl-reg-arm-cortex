...
struct MCG
{
  ...
  : public reg< uint8_t, base_addr + 0xc, rw, 0 >
  {
    ...
    using OSCSEL  = regbits< type,  0,  2 >;  /**< MCG OSC Clock Select  */
  };
   ...
   */
  struct C9
  : public reg< uint8_t, base_addr + 0xe, rw, 0x10 >
  {
    using type = reg< uint8_t, base_addr + 0xe, rw, 0x10 >;

    using EXT_PLL_LOCS  = regbits< type,  0,  1 >;  /**< External PLL Loss of Clock Status            */
    using PLL_LOCRE     = regbits< type,  4,  1 >;  /**< MCG External PLL Loss of Clock Reset Enable  */
    using PLL_CME       = regbits< type,  5,  1 >;  /**< MCG External PLL Clock Monitor Enable        */
  };
