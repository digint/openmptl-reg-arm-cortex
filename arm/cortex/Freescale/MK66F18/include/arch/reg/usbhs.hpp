...
struct USBHS
{
  static constexpr reg_addr_t base_addr = 0x400a1000;
  ...
  : public reg< uint32_t, base_addr + 0, ro, 0xE461FA05 >
  {
    ...
    using NID        = regbits< type,  8,  6 >;  /**< Ones complement version of ID.  */
   ...
   */
  struct HWGENERAL
  : public reg< uint32_t, base_addr + 0x4, ro, 0x15 >
  {
    using type = reg< uint32_t, base_addr + 0x4, ro, 0x15 >;

    using PHYW  = regbits< type,  4,  2 >;  /**< PHY Width    */
  ...
  : public reg< uint32_t, base_addr + 0x80, rw, 0 >
  {
    ...
    using GPTLD  = regbits< type,  0, 24 >;  /**< Specifies the value to be loaded into the countdown timer on a reset  */
  };
  ...
  : public reg< uint16_t, base_addr + 0x122, ro, 0x1 >
  {
    ...
    using DCIVERSION_ = regbits< type,  0, 16 >;  /**< Device interface revision number.  */
  };
   ...
   */
  struct BURSTSIZE
  : public reg< uint32_t, base_addr + 0x160, rw, 0x808 >
  {
    using type = reg< uint32_t, base_addr + 0x160, rw, 0x808 >;
  ...
  : public reg< uint32_t, base_addr + 0x184, rw, 0 >
  {
    ...
    using PTS2  = regbits< type, 25,  1 >;  /**< Port Transceiver Select [2]    */
    using PSPD  = regbits< type, 26,  2 >;  /**< Port Speed                     */
    using PTS   = regbits< type, 30,  2 >;  /**< Port Transceiver Select [1:0]  */
  };
