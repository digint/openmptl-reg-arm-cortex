...
#ifndef ARCH_REG_USBHSDCD_HPP_INCLUDED
#define ARCH_REG_USBHSDCD_HPP_INCLUDED
...
struct USBHSDCD
{
  ...
  : public reg< uint32_t, base_addr + 0, rw, 0x10000 >
  {
    ...
    using BC12   = regbits< type, 17,  1 >;  /**< BC1.2 compatibility. This bit cannot be changed after start detection.  */
  ...
  : public reg< uint32_t, base_addr + 0xc, rw, 0 >
  {
    ...
    using PS  = regbits< type,  0,  2 >;  /**< Phase Selection  */
  };
   ...
   */
  struct TIMER2_BC12
  : public reg< uint32_t, base_addr + 0x18, rw, 0x10028 >
  {
    using type = reg< uint32_t, base_addr + 0x18, rw, 0x10028 >;

    using TVDMSRC_ON       = regbits< type,  0, 10 >;  /**< Sets the amount of time (in ms) that the module enables the VDM_SRC. Valid values are 0-40ms.                      */
    using TWAIT_AFTER_PRD  = regbits< type, 16, 10 >;  /**< Sets the amount of time (in ms) that the module waits after primary detection before start to secondary detection  */
  };
};
} // namespace mptl

#endif // ARCH_REG_USBHSDCD_HPP_INCLUDED
