...
struct FTFE
{
   ...
   */
  struct FCNFG
  : public reg< uint8_t, base_addr + 0x1, rw, 0x2 >
  {
    using type = reg< uint8_t, base_addr + 0x1, rw, 0x2 >;

    using EEERDY    = regbits< type,  0,  1 >;  /**< This bit is reserved and always has the value 0.  */
    using RAMRDY    = regbits< type,  1,  1 >;  /**< RAM Ready                                         */
    using PFLSH     = regbits< type,  2,  1 >;  /**< This bit is reserved and always has the value 0.  */
  ...
  : public reg< uint8_t, base_addr + 0x2, ro, 0 >
  {
    ...
    using FSLACC  = regbits< type,  2,  2 >;  /**< Factory Security Level Access Code  */
