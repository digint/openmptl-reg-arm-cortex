...
struct EWM
{
  ...
  : public reg< uint8_t, base_addr + 0x1, wo, 0 >
  {
    ...
    using SERVICE  = regbits< type,  0,  8 >;  /**< The EWM refresh mechanism requires the CPU to write two values to the SERV register: a first data byte of 0xB4, followed by a second data byte of 0x2C  */
  };
  ...
  : public reg< uint8_t, base_addr + 0x2, rw, 0 >
  {
    ...
    using COMPAREL  = regbits< type,  0,  8 >;  /**< To prevent runaway code from changing this field, software should write to this field after a CPU reset even if the (default) minimum refresh time is required  */
  };
  ...
  : public reg< uint8_t, base_addr + 0x3, rw, 0xFF >
  {
    ...
    using COMPAREH  = regbits< type,  0,  8 >;  /**< To prevent runaway code from changing this field, software should write to this field after a CPU reset even if the (default) maximum refresh time is required  */
  };
