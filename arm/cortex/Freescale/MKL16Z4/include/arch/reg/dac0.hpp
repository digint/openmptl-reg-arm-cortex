...
struct DAC0
{
  ...
  : public reg< uint8_t, base_addr + 0, rw, 0 >
  {
    ...
    using DATA0  = regbits< type,  0,  8 >;  /**< When the DAC buffer is not enabled, DATA[11:0] controls the output voltage based on the following formula: V out = V in * (1 + DACDAT0[11:0])/4096 When the DAC buffer is enabled, DATA is mapped to the 16-word buffer  */
  };
  ...
  : public reg< uint8_t, base_addr + 0x1, rw, 0 >
  {
    ...
    using DATA1  = regbits< type,  0,  4 >;  /**< When the DAC Buffer is not enabled, DATA[11:0] controls the output voltage based on the following formula  */
  };
  ...
  : public reg< uint8_t, base_addr + 0x22, rw, 0 >
  {
    ...
    using DACBFMD  = regbits< type,  2,  1 >;  /**< DAC Buffer Work Mode Select  */
   ...
   */
  struct C2
  : public reg< uint8_t, base_addr + 0x23, rw, 0x1 >
  {
    using type = reg< uint8_t, base_addr + 0x23, rw, 0x1 >;

    using DACBFUP  = regbits< type,  0,  1 >;  /**< DAC Buffer Upper Limit   */
    using DACBFRP  = regbits< type,  4,  1 >;  /**< DAC Buffer Read Pointer  */
  };
};
} // namespace mptl
