...
struct WDOG
{
   ...
   */
  struct STCTRLH
  : public reg< uint16_t, base_addr + 0, rw, 0x1D1 >
  {
    using type = reg< uint16_t, base_addr + 0, rw, 0x1D1 >;
   ...
   */
  struct TOVALL
  : public reg< uint16_t, base_addr + 0x6, rw, 0x4B40 >
  {
    using type = reg< uint16_t, base_addr + 0x6, rw, 0x4B40 >;
