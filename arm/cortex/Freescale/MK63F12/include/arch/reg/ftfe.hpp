...
struct FTFE
{
  ...
  : public reg< uint8_t, base_addr + 0x1, rw, 0 >
  {
    ...
    using EEERDY    = regbits< type,  0,  1 >;  /**< During the reset sequence, the EEERDY flag remains clear while CCIF=0 and only sets if the FlexNVM block is partitioned for EEPROM  */
