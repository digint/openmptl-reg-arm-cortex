...
struct SMC
{
  ...
  : public reg< uint8_t, base_addr + 0x3, ro, 0x1 >
  {
    ...
    using PMSTAT_ = regbits< type,  0,  8 >;  /**< When debug is enabled, the PMSTAT will not update to STOP or VLPS When a PSTOP mode is enabled, the PMSTAT will not update to STOP or VLPS  */
  };
};
} // namespace mptl
