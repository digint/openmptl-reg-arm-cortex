...
struct WDOG
{
  ...
  : public reg< uint16_t, base_addr + 0, rw, 0x1D3 >
  {
    ...
    using WINEN        = regbits< type,  3,  1 >;  /**< Enable windowing mode.                                                                                                      */
    using ALLOWUPDATE  = regbits< type,  4,  1 >;  /**< Enables updates to watchdog write once registers, after initial configuration window (WCT) closes, through unlock sequence  */
    ...
    using TESTSEL      = regbits< type, 11,  1 >;  /**< Selects the test to be run on the watchdog timer. Effective only if TESTWDOG is set.                                        */
    using BYTESEL      = regbits< type, 12,  2 >;  /**< This 2-bit field select the byte to be tested when the watchdog is in the byte test mode.                                   */
  ...
  : public reg< uint16_t, base_addr + 0xe, rw, 0xD928 >
  {
    ...
    using WDOGUNLOCK  = regbits< type,  0, 16 >;  /**< You can write the unlock sequence values to this register to make the watchdog write once registers writable again  */
  };
