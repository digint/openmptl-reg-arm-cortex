...
struct LLWU
{
  ...
  : public reg< uint8_t, base_addr + 0x7, rw, 0 >
  {
    ...
    using MWUF7  = regbits< type,  7,  1 >;  /**< Wakeup flag for module 7 (Error Detect)  */
  };
   ...
   */
  struct CS
  : public reg< uint8_t, base_addr + 0x8, rw, 0x4 >
  {
    using type = reg< uint8_t, base_addr + 0x8, rw, 0x4 >;

    using FLTR    = regbits< type,  0,  1 >;  /**< Digital Filter on RESET Pin     */
    using FLTEP   = regbits< type,  1,  1 >;  /**< Digital Filter on External Pin  */
    using ACKISO  = regbits< type,  7,  1 >;  /**< Acknowledge Isolation           */
  };
};
} // namespace mptl
