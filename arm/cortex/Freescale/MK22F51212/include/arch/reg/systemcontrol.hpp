...
struct SystemControl
{
   ...
   */
  struct FPCCR
  : public reg< uint32_t, base_addr + 0xf34, rw, 0xC0000000 >
  {
    using type = reg< uint32_t, base_addr + 0xf34, rw, 0xC0000000 >;

    using LSPACT  = regbits< type,  0,  1 >;  /**< Lazy state preservation.                                                                                                                                                                                */
    using USER    = regbits< type,  1,  1 >;  /**< Privilege level when the floating-point stack frame was allocated.                                                                                                                                      */
    using THREAD  = regbits< type,  3,  1 >;  /**< Mode when the floating-point stack frame was allocated.                                                                                                                                                 */
    using HFRDY   = regbits< type,  4,  1 >;  /**< Permission to set the HardFault handler to the pending state when the floating-point stack frame was allocated.                                                                                         */
    using MMRDY   = regbits< type,  5,  1 >;  /**< Permission to set the MemManage handler to the pending state when the floating-point stack frame was allocated.                                                                                         */
    using BFRDY   = regbits< type,  6,  1 >;  /**< Permission to set the BusFault handler to the pending state when the floating-point stack frame was allocated.                                                                                          */
    using MONRDY  = regbits< type,  8,  1 >;  /**< Permission to set the MON_PEND when the floating-point stack frame was allocated.                                                                                                                       */
    using LSPEN   = regbits< type, 30,  1 >;  /**< Lazy state preservation for floating-point context.                                                                                                                                                     */
    using ASPEN   = regbits< type, 31,  1 >;  /**< Enables CONTROL2 setting on execution of a floating-point instruction. This results in automatic hardware state preservation and restoration, for floating-point context, on exception entry and exit.  */
  };
   ...
   */
  struct FPCAR
  : public reg< uint32_t, base_addr + 0xf38, rw, 0 >
  {
    using type = reg< uint32_t, base_addr + 0xf38, rw, 0 >;

    using ADDRESS  = regbits< type,  3, 29 >;  /**< The location of the unpopulated floating-point register space allocated on an exception stack frame.  */
  };
   ...
   */
  struct FPDSCR
  : public reg< uint32_t, base_addr + 0xf3c, rw, 0 >
  {
    using type = reg< uint32_t, base_addr + 0xf3c, rw, 0 >;

    using RMode  = regbits< type, 22,  2 >;  /**< Default value for FPSCR.RMode (Rounding Mode control field).           */
    using FZ     = regbits< type, 24,  1 >;  /**< Default value for FPSCR.FZ (Flush-to-zero mode control bit).           */
    using DN     = regbits< type, 25,  1 >;  /**< Default value for FPSCR.DN (Default NaN mode control bit).             */
    using AHP    = regbits< type, 26,  1 >;  /**< Default value for FPSCR.AHP (Alternative half-precision control bit).  */
  };
};
} // namespace mptl
