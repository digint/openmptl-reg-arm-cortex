...
struct SPI0
{
  ...
  : public reg< uint32_t, base_addr + 0, rw, 0x4001 >
  {
    ...
    using CLR_RXF    = regbits< type, 10,  1 >;  /**< CLR_RXF                                  */
    ...
    using PCSIS      = regbits< type, 16,  5 >;  /**< Peripheral Chip Select x Inactive State  */
    using ROOE       = regbits< type, 24,  1 >;  /**< Receive FIFO Overflow Overwrite Enable   */
    using MTFE       = regbits< type, 26,  1 >;  /**< Modified Transfer Format Enable          */
    using FRZ        = regbits< type, 27,  1 >;  /**< Freeze                                   */
    using DCONF      = regbits< type, 28,  2 >;  /**< SPI Configuration.                       */
  ...
  : public reg< uint32_t, base_addr + 0xc, rw, 0x78000000 >
  {
    ...
    using LSBFE   = regbits< type, 24,  1 >;  /**< LSB First                       */
  ...
  : public reg< uint32_t, base_addr + 0x30, rw, 0 >
  {
    ...
    using EOQF_RE    = regbits< type, 28,  1 >;  /**< Finished Request Enable                             */
  ...
  : public reg< uint32_t, base_addr + 0x34, rw, 0 >
  {
    ...
    using PCS     = regbits< type, 16,  5 >;  /**< Select which PCS signals are to be asserted for the transfer  */
   ...
   */
  struct SREX
  : public reg< uint32_t, base_addr + 0x13c, ro, 0 >
  {
    using type = reg< uint32_t, base_addr + 0x13c, ro, 0 >;

    using CMDNXTPTR  = regbits< type,  0,  4 >;  /**< Command Next Pointer  */
    using CMDCTR     = regbits< type,  4,  5 >;  /**< CMD FIFO Counter      */
    using TXCTR4     = regbits< type, 11,  1 >;  /**< TX FIFO Counter[4]    */
    using RXCTR4     = regbits< type, 14,  1 >;  /**< RX FIFO Counter[4]    */
  };
};
} // namespace mptl
