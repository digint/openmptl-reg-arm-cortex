...
struct MCM
{
   ...
   */
  struct PLAMC
  : public reg< uint16_t, base_addr + 0xa, ro, 0x5 >
  {
    using type = reg< uint16_t, base_addr + 0xa, ro, 0x5 >;
   ...
   */
  struct PLACR
  : public reg< uint32_t, base_addr + 0xc, rw, 0x250 >
  {
    using type = reg< uint32_t, base_addr + 0xc, rw, 0x250 >;
  ...
  : public reg< uint32_t, base_addr + 0x30, rw, 0 >
  {
    ...
    using PID_ = regbits< type,  0,  8 >;  /**< M0_PID For MPU  */
  };
  ...
  : public reg< uint32_t, base_addr + 0x80, rw, 0 >
  {
    ...
    using ATC0  = regbits< type,  0,  3 >;  /**< Attribute Configuration Master n  */
    using RO0   = regbits< type,  7,  1 >;  /**< Read-Only Master n                */
    using ATC2  = regbits< type, 16,  3 >;  /**< Attribute Configuration Master n  */
    using RO2   = regbits< type, 23,  1 >;  /**< Read-Only Master n                */
  };
};
} // namespace mptl
