...
struct MCG
{
   ...
   */
  struct C1
  : public reg< uint8_t, base_addr + 0, rw, 0x44 >
  {
    using type = reg< uint8_t, base_addr + 0, rw, 0x44 >;
   ...
   */
  struct C2
  : public reg< uint8_t, base_addr + 0x1, rw, 0x83 >
  {
    using type = reg< uint8_t, base_addr + 0x1, rw, 0x83 >;
  ...
  : public reg< uint8_t, base_addr + 0x4, rw, 0 >
  {
    ...
    using PLLSTEN0   = regbits< type,  5,  1 >;  /**< PLL Stop Enable   */
   ...
   */
  struct C6
  : public reg< uint8_t, base_addr + 0x5, rw, 0x8 >
  {
    using type = reg< uint8_t, base_addr + 0x5, rw, 0x8 >;

    using CHGPMP_BIAS  = regbits< type,  0,  5 >;  /**< Directly controls the PLL Charge Pump Current. Appropiate selection of this value is imperative to ensure stable operation of the PLL closed loop system. The default value for this field is set to 5'b01000 out of reset which generates a nominal 750 nA charge pump current (lcp).  */
   ...
   */
  struct S
  : public reg< uint8_t, base_addr + 0x6, rw, 0x15 >
  {
    using type = reg< uint8_t, base_addr + 0x6, rw, 0x15 >;
  ...
  : public reg< uint8_t, base_addr + 0xc, rw, 0 >
  {
    ...
    using PLL32KREFSEL  = regbits< type,  6,  2 >;  /**< MCG PLL 32Khz Reference Clock Select  */
  };
  ...
  : public reg< uint8_t, base_addr + 0xd, rw, 0x80 >
  {
    ...
    using COARSE_LOLIE  = regbits< type,  4,  1 >;  /**< Loss of Coarse Lock Interrrupt Enable  */
  ...
  : public reg< uint8_t, base_addr + 0xe, ro, 0 >
  {
    ...
    using COARSE_LOCK  = regbits< type,  6,  1 >;  /**< Coarse Lock Status          */
    using COARSE_LOLS  = regbits< type,  7,  1 >;  /**< Coarse Loss of Lock Status  */
  };
};
} // namespace mptl
