...
struct VREF
{
   ...
   */
  struct VREFL_TRM
  : public reg< uint8_t, base_addr + 0x5, rw, 0x3 >
  {
    using type = reg< uint8_t, base_addr + 0x5, rw, 0x3 >;

    using VREFL_TRIM  = regbits< type,  0,  3 >;  /**< These bits trim the VREFL reference voltage in steps of 10 mV    */
    using VREFL_EN    = regbits< type,  3,  1 >;  /**< This bit enables the VREFL (0.4 V) reference buffer.             */
    using VREFL_SEL   = regbits< type,  4,  1 >;  /**< This bit selects between internal and external 1.2 V reference.  */
  };
};
} // namespace mptl
