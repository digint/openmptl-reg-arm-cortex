...
struct ENET
{
  ...
  : public reg< uint32_t, base_addr + 0x118, rw, 0 >
  {
    ...
    using IADDR1  = regbits< type,  0, 32 >;  /**< The upper 32 bits of the 64-bit hash table used in the address recognition process for receive frames with a unicast address  */
  };
  ...
  : public reg< uint32_t, base_addr + 0x11c, rw, 0 >
  {
    ...
    using IADDR2  = regbits< type,  0, 32 >;  /**< The lower 32 bits of the 64-bit hash table used in the address recognition process for receive frames with a unicast address  */
  };
  ...
  : public reg< uint32_t, base_addr + 0x180, rw, 0 >
  {
    ...
    using R_DES_START  = regbits< type,  3, 29 >;  /**< Pointer to the start of the receive buffer descriptor queue.  */
  };
  ...
  : public reg< uint32_t, base_addr + 0x184, rw, 0 >
  {
    ...
    using X_DES_START  = regbits< type,  3, 29 >;  /**< Pointer to the start of the transmit buffer descriptor queue.  */
  };
