...
struct DDR
{
  ...
  : public reg< uint32_t, base_addr + 0x10, rw, 0 >
  {
    ...
    using TRRD     = regbits< type,  8,  3 >;  /**< Defines the DRAM activate-to-activate delay for different banks (TRRD) in cycles.   */
    using TRC      = regbits< type, 16,  6 >;  /**< Defines the DRAM period between active commands for the same bank (TRC) in cycles.  */
  ...
  : public reg< uint32_t, base_addr + 0x14, rw, 0 >
  {
    ...
    using TRP   = regbits< type,  8,  4 >;  /**< Defines the DRAM precharge command time (TRP) in cycles.  */
    using TRTP  = regbits< type, 16,  3 >;  /**< Time Read-To-Precharge                                    */
    using TMRD  = regbits< type, 24,  5 >;  /**< DRAM TMRD parameter in cycles.                            */
  };
  ...
  : public reg< uint32_t, base_addr + 0x20, rw, 0 >
  {
    ...
    using TDAL    = regbits< type, 24,  5 >;  /**< Defines the auto-precharge write recovery time when auto-precharge is enabled (CR01[AP] is set), in cycles  */
  };
  ...
  : public reg< uint32_t, base_addr + 0x2c, rw, 0 >
  {
    ...
    using TREFEN    = regbits< type, 24,  1 >;  /**< Enables refresh commands  */
  };
  ...
  : public reg< uint32_t, base_addr + 0x50, rw, 0xC000000 >
  {
    ...
    using CKSRE  = regbits< type,  8,  4 >;  /**< Clock hold delay on self refresh entry  */
  ...
  : public reg< uint32_t, base_addr + 0x54, rw, 0x400 >
  {
    ...
    using MR0DAT0  = regbits< type,  0, 16 >;  /**< Data to program into memory mode register 0 for chip select .  */
    using MR1DAT0  = regbits< type, 16, 16 >;  /**< Data to program into memory mode register 1 for chip select .  */
  };
  ...
  : public reg< uint32_t, base_addr + 0x58, rw, 0x400 >
  {
    ...
    using MR2DATA0  = regbits< type,  0, 16 >;  /**< Data to program into memory mode register 2 for chip select .  */
    using MR3DAT0   = regbits< type, 16, 16 >;  /**< Data to program into memory mode register 3 for chip select .  */
  };
  ...
  : public reg< uint32_t, base_addr + 0x70, rw, 0 >
  {
    ...
    using REDUC    = regbits< type,  8,  1 >;  /**< Controls the width of the memory datapath  */
